...
Deno.serve(async (req) => {
  ...
  const weekTrades = allTrades.filter(t => new Date(t.created_date) >= sevenDaysAgo);
...
<body>
  <div class="container">
    ...
    <div class="stat">
      ...
      <div class="stat-value">${weekTrades.length}</div>
    </div>
    ...
    <div class="footer">
      <p>Weiter so! Disziplin schlägt Talent. Jeden Tag.</p>
      <p>— ZNPCV Team</p>
    </div>
  </div>
</body>
  ...
  return Response.json({ 
    ...
    stats: { weekTrades: weekTrades.length, winRate, totalPnL, wins, losses }
  });
});