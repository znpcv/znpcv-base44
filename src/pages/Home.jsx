    ...
    <div className={`min-h-screen ${theme.bg} ${theme.text} ${isRTL ? 'rtl' : 'ltr'}`}>
      ...
      <main className="max-w-6xl mx-auto px-2 sm:px-3 md:px-6 py-3 sm:py-4 md:py-8 lg:py-12">
        ...
        <motion.div
          ...
          className="grid grid-cols-2 gap-2 sm:gap-2.5 md:gap-4 lg:gap-6 mb-4 sm:mb-6 md:mb-10 lg:mb-14"
        >
          {/* New Analysis - Compact */}
          <button
            onClick={() => navigate(createPageUrl('Checklist'))}
            className={cn("group relative p-3 sm:p-4 md:p-5 lg:p-8 rounded-xl sm:rounded-2xl md:rounded-3xl hover:shadow-2xl transition-all text-left overflow-hidden",
              darkMode ? "bg-white text-black" : "bg-black text-white")}
          >
            {/* Decorative elements */}
            <div className="absolute top-0 right-0 w-16 h-16 sm:w-24 sm:h-24 md:w-32 md:h-32 lg:w-40 lg:h-40 bg-black/5 rounded-full -translate-y-8 sm:-translate-y-12 md:-translate-y-16 lg:-translate-y-20 translate-x-8 sm:translate-x-12 md:translate-x-16 lg:translate-x-20" />
            <div className="absolute bottom-0 left-0 w-12 h-12 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-32 lg:h-32 bg-black/5 rounded-full translate-y-6 sm:translate-y-10 md:translate-y-12 lg:translate-y-16 -translate-x-6 sm:-translate-x-10 md:-translate-x-12 lg:-translate-x-16" />

            <div className="relative z-10">
              <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 mb-2 sm:mb-3 md:mb-4 lg:mb-6">
                <div className={cn("w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 rounded-lg sm:rounded-xl md:rounded-2xl flex items-center justify-center",
                  darkMode ? "bg-black" : "bg-white")}>
                  <ClipboardCheck className={cn("w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-6 lg:h-6", darkMode ? "text-white" : "text-black")} />
                </div>
                <div className="flex items-center gap-0.5 sm:gap-1 px-1.5 py-0.5 sm:px-2 sm:py-0.5 md:px-2.5 md:py-1 bg-teal-600 text-white text-[9px] sm:text-[10px] md:text-xs rounded-full ml-auto">
                  <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-white rounded-full animate-pulse" />
                  <span className="hidden sm:inline">START</span>
                </div>
              </div>

              <h2 className={cn("text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl tracking-wider mb-2 sm:mb-3 md:mb-4 font-bold", darkMode ? "text-black" : "text-white")}>{t('newAnalysis')}</h2>

              <div className={cn("flex items-center gap-1.5 sm:gap-2 md:gap-3 font-bold tracking-widest", darkMode ? "text-black" : "text-white")}>
                <span className="text-xs sm:text-sm md:text-base lg:text-lg">{t('startNow')}</span>
                <div className={cn("w-6 h-6 sm:w-7 sm:h-7 md:w-9 md:h-9 lg:w-10 lg:h-10 rounded-md sm:rounded-lg md:rounded-xl flex items-center justify-center group-hover:translate-x-2 transition-transform",
          ...
          <button
            ...
            className={cn("group relative p-3 sm:p-4 md:p-5 lg:p-8 rounded-xl sm:rounded-2xl md:rounded-3xl hover:shadow-2xl transition-all text-left overflow-hidden border-2",
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 mb-2 sm:mb-3 md:mb-4 lg:mb-6">
                <div className={cn("w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 rounded-lg sm:rounded-xl md:rounded-2xl flex items-center justify-center",
                  ...
                  <BarChart3 className={cn("w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-6 lg:h-6", darkMode ? "text-black" : "text-white")} />
                </div>
                <div className="flex items-center gap-0.5 sm:gap-1 px-1.5 py-0.5 sm:px-2 sm:py-0.5 md:px-2.5 md:py-1 bg-zinc-800 text-zinc-300 text-[9px] sm:text-[10px] md:text-xs rounded-full ml-auto">
                  <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-blue-500 rounded-full animate-pulse" />
                  <span className="hidden sm:inline">LIVE</span>
                </div>
              </div>

              <h2 className={cn("text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl tracking-wider mb-2 sm:mb-3 md:mb-4 font-bold", darkMode ? "text-white" : "text-black")}>{t('dashboard')}</h2>

              <div className={cn("flex items-center gap-1.5 sm:gap-2 md:gap-3 font-bold tracking-widest", darkMode ? "text-white" : "text-black")}>
                <span className="text-xs sm:text-sm md:text-base lg:text-lg">{t('openDashboard')}</span>
                <div className={cn("w-6 h-6 sm:w-7 sm:h-7 md:w-9 md:h-9 lg:w-10 lg:h-10 rounded-md sm:rounded-lg md:rounded-xl flex items-center justify-center group-hover:translate-x-2 transition-transform",
                  darkMode ? "bg-white text-black" : "bg-black text-white")}>
                  <ArrowRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 lg:w-5 lg:h-5" />
                </div>
