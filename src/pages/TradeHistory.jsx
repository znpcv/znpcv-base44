...
export default function TradeHistoryPage() {
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} ${isRTL ? 'rtl' : 'ltr'}`}>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-5 md:py-6 lg:py-10">
        {/* Title mit Actions */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.1 }} className="mb-4 sm:mb-5 md:mb-6 lg:mb-8">
          <div className="flex items-start justify-between gap-3 sm:gap-4 md:gap-5 mb-4 sm:mb-5">
            <div className="flex items-center gap-2 sm:gap-3">
              <button
                onClick={() => navigate(createPageUrl('Dashboard'))}
                className={`${darkMode ? 'bg-zinc-900 border-zinc-800 hover:border-zinc-700' : 'bg-zinc-100 border-zinc-300 hover:border-zinc-400'} border-2 rounded-xl p-2 sm:p-2.5 md:p-3 transition-all flex-shrink-0 group`}>
                <ArrowLeft className={`w-4 h-4 sm:w-5 sm:h-5 ${theme.text} group-hover:-translate-x-1 transition-transform`} />
              </button>
              <div className="min-w-0">
                <h1 className={`text-lg sm:text-xl md:text-2xl lg:text-3xl tracking-widest mb-1 ${theme.text}`}>{t('tradeHistory')}</h1>
                <p className={`${theme.textMuted} text-[10px] sm:text-xs tracking-wider`}>{t('performanceAnalytics')}</p>
              </div>
            </div>
            
            {/* Export Buttons */}
            <div className="flex gap-1.5 sm:gap-2 flex-shrink-0">
              <Button onClick={handleExportPDF} disabled={exporting} variant="outline" className={cn("h-8 sm:h-9 px-2 sm:px-3 border-2 text-xs", theme.border, darkMode ? 'hover:bg-zinc-900' : 'hover:bg-zinc-100')}>
                <Download className="w-3.5 h-3.5" />
                <span className="hidden sm:inline sm:ml-1.5">PDF</span>
              </Button>
              <Button onClick={handleExportExcel} disabled={exporting} variant="outline" className={cn("h-8 sm:h-9 px-2 sm:px-3 border-2 text-xs", theme.border, darkMode ? 'hover:bg-zinc-900' : 'hover:bg-zinc-100')}>
                <FileText className="w-3.5 h-3.5" />
                <span className="hidden sm:inline sm:ml-1.5">CSV</span>
              </Button>
              ...
              <Button
                ...
                compareMode ? "bg-teal-600 text-white border-teal-600" : theme.border)}>
                  ...
                  <span className="hidden sm:inline sm:ml-1.5">{compareMode ? 'Fertig' : 'Vergleich'}</span>
                </Button>
              }
            </div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.1 }} className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5 lg:gap-6 mb-5 sm:mb-6 md:mb-8">
          <div className={cn("border-2 rounded-xl md:rounded-2xl p-4 sm:p-5 md:p-6 lg:p-8", stats.totalPnL >= 0 ? "bg-teal-600 border-teal-600 text-white" : "bg-rose-600 border-rose-600 text-white")}>
            <TrendingUp className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 mb-3 sm:mb-4" />
          ...
          <div className={`border-2 ${theme.border} rounded-xl md:rounded-2xl p-4 sm:p-5 md:p-6 lg:p-8 ${theme.bgSecondary}`}>
            <Award className={`w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 mb-3 sm:mb-4 ${theme.text}`} />
          ...
          <div className={`border-2 ${theme.border} rounded-xl md:rounded-2xl p-4 sm:p-5 md:p-6 lg:p-8 ${theme.bgSecondary}`}>
            <Target className={`w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 mb-3 sm:mb-4 ${theme.text}`} />
          ...
          <div className={`border-2 ${theme.border} rounded-xl md:rounded-2xl p-4 sm:p-5 md:p-6 lg:p-8 ${theme.bgSecondary}`}>
            <Calendar className={`w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 mb-3 sm:mb-4 ${theme.text}`} />
            <div className={`text-xl sm:text-2xl md:text-3xl lg:text-4xl font-light mb-1 sm:mb-2 ${theme.text}`}>{stats.executed}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            className={`border-2 ${theme.border} rounded-xl md:rounded-2xl ${theme.bgSecondary} overflow-hidden`}>
              <div className={`p-4 sm:p-5 md:p-6 border-b ${theme.border}`}>
              ...
              {isLoading ?
              <div className={`p-8 text-center ${theme.textDimmed} text-xs sm:text-sm`}>{t('loading')}</div> :
              filteredTrades.length === 0 ?
              <div className="p-8 text-center">
                  <p className={`${theme.textDimmed} mb-4 text-xs sm:text-sm`}>{t('noAnalyses')}</p>
                  <Button onClick={() => navigate(createPageUrl('Checklist'))} className={`rounded-xl border-2 font-bold px-4 sm:px-6 h-9 sm:h-10 text-xs ${darkMode ? 'bg-white text-black border-white hover:bg-zinc-100' : 'bg-zinc-900 text-white border-zinc-900 hover:bg-zinc-800'}`}>
                    {t('startFirstAnalysis')}
                  </Button>
                </div> :

              <div className={`divide-y ${darkMode ? 'divide-zinc-800/30' : 'divide-zinc-200'} max-h-[450px] sm:max-h-[500px] md:max-h-[600px] overflow-y-auto`}>
                  {filteredTrades.map((trade) =>
                <div key={trade.id}
                className={cn("p-3 sm:p-4 md:p-5 transition-all group cursor-pointer relative",
                    ...
                    navigate(createPageUrl('TradeDetail') + `?id=${trade.id}`);
                      ...
                      <div
                        ...
                        darkMode ? "border-zinc-700 hover:border-zinc-600" : "border-zinc-400 hover:border-zinc-500")}>
                          ...
                          <div className={cn("w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 flex items-center justify-center rounded-xl border-2",
                      ...
                      trade.direction === 'long' ? 'border-2 border-teal-600 text-teal-600' : 'border-2 border-rose-600 text-rose-600')}>
                        ...
                        <ArrowUpRight className="w-5 h-5 sm:w-5.5 sm:h-5.5 md:w-6 md:h-6" /> : <ArrowDownRight className="w-5 h-5 sm:w-5.5 sm:h-5.5 md:w-6 md:h-6" />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className={`text-sm sm:text-base md:text-lg font-bold tracking-wider ${theme.text} mb-0.5 truncate`}>{trade.pair || '-'}</div>
                            <div className={`text-[10px] sm:text-xs ${theme.textMuted}`}>{format(new Date(trade.trade_date || trade.created_date), 'dd.MM.yyyy')}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <>
                                <div className={cn("text-sm sm:text-base md:text-lg lg:text-xl font-bold mb-0.5",
                          ...
                          parseFloat(trade.actual_pnl) < 0 ? 'text-rose-600' : theme.text)}>
                                ...
                                <div className={cn("text-[9px] sm:text-[10px] tracking-wider px-1.5 py-0.5 rounded-full",
                          ...
                          trade.outcome === 'loss' ? 'bg-rose-600/20 text-rose-600' : 'bg-zinc-600/20 text-zinc-400')}>
                            ...
                            {!trade.outcome &&
                        <span className="px-2 py-0.5 sm:py-1 bg-blue-500 text-white text-[9px] sm:text-[10px] tracking-wider rounded-full font-bold">
                        ...
                        className={`p-1.5 rounded-lg transition-colors ${darkMode ? 'hover:bg-teal-600/20 text-teal-400' : 'hover:bg-teal-100 text-teal-600'}`}>
                              <Edit className="w-3.5 h-3.5" />
                            </button>
                            <button type="button" onClick={(e) => handleDeleteTrade(e, trade.id)}
                        className={`p-1.5 rounded-lg transition-colors ${darkMode ? 'hover:bg-rose-600/20 text-rose-400' : 'hover:bg-red-100 text-red-600'}`}>
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 text-[9px] sm:text-[10px] flex-wrap">
                        ...
                        {trade.entry_price && <span className={`hidden sm:inline ${theme.textMuted}`}>{t('entry')}: <span className={`font-bold ${theme.text}`}>{trade.entry_price}</span></span>}
                      </div>
                    </div>
                )}
                </div>
          ...
          <div className="space-y-4 sm:space-y-5">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.1 }}
            className={`border-2 ${theme.border} rounded-xl md:rounded-2xl p-4 sm:p-5 md:p-6 ${theme.bgSecondary}`}>
              <h3 className={`text-sm sm:text-base tracking-widest mb-3 sm:mb-4 ${theme.text}`}>{t('winLoss')}</h3>
        ...
        {/* Footer */}
        <footer className={`mt-16 sm:mt-20 md:mt-24 border-t ${theme.border}`}>
          <div className="py-8 sm:py-10">
            <div className="flex flex-col items-center gap-4 mb-6">
              <img src={darkMode ?
              "https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/692d8f74cb6d9152b3880015/e14bd7c71_ZNPCVSchwarzhintergrundlogochecklisteweb.png" :
              "https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/692d8f74cb6d9152b3880015/e396a6edd_ZNPCVWebseiteWeisshihtergrundLogo.png"
              } alt="ZNPCV" className="h-12 sm:h-16 w-auto opacity-40" />
              <p className={`${theme.textDimmed} text-xs tracking-widest`}>© {new Date().getFullYear()} ZNPCV</p>
            </div>
