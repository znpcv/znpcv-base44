...
export default function AdvancedMetrics({ checklists, darkMode }) {
  ...
  return (
    ...
    >
      <div className={`border ${theme.border} rounded-2xl p-4 ${theme.bg}`}>
        <Zap className={`w-5 h-5 mb-2 ${theme.text}`} />
        <div className={`text-2xl font-light mb-1 ${theme.text}`}>{metrics.profitFactor}</div>
        <div className={`text-[10px] tracking-widest ${theme.textMuted}`}>PROFIT FACTOR</div>
      </div>
      <div className={`border ${theme.border} rounded-2xl p-4 ${theme.bg}`}>
        <Target className={`w-5 h-5 mb-2 ${theme.text}`} />
        <div className={`text-2xl font-light mb-1 ${theme.text}`}>1:{metrics.avgRR}</div>
        <div className={`text-[10px] tracking-widest ${theme.textMuted}`}>AVG R:R</div>
      </div>
      <div className={`border ${theme.border} rounded-2xl p-4 ${theme.bg}`}>
        <Award className={`w-5 h-5 mb-2 ${theme.text}`} />
        <div className={`text-2xl font-light mb-1 ${theme.text}`}>{metrics.avgScore}%</div>
        <div className={`text-[10px] tracking-widest ${theme.textMuted}`}>AVG SCORE</div>
      </div>
      <div className={`border ${theme.border} rounded-2xl p-4 ${theme.bg}`}>
        <Activity className={`w-5 h-5 mb-2 ${theme.text}`} />
        <div className={`text-2xl font-light mb-1 ${theme.text}`}>{metrics.roi}%</div>
        <div className={`text-[10px] tracking-widest ${theme.textMuted}`}>ROI</div>
      </div>
    </motion.div>
  );
}